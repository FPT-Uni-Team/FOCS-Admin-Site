...
import {
  assignCouponsStart,
  resetAssignState,
} from "../../../store/slices/coupon/couponAssignSlice";
...
const CouponAssignForm: React.FC<CouponAssignFormProps> = ({
  ...
  } = useAppSelector((state) => state.couponAssign);
      ...
      const loadPromotions = async () => {
        try {
          ...
          const response = await axiosClient.post(
            endpoints.promotion.get(storeId),
            {
              page: 1,
              page_size: 50,
              search_by: "",
          ...
          message.warning(
            "Unable to load promotion list from API, using sample data"
          );
        } finally {
      ...
      <Form form={form} layout="vertical" style={{ marginTop: 16 }}>
        ...
        >
          {promotionId ? (
            ...
            <Select
              ...
              notFoundContent={
                promotionLoading ? "Đang tải..." : "Không có promotion nào"
              }
            >
        ...
        <Form.Item
          label={
            <Space>
              ...
              <Text type="secondary">
                ({availableCoupons.length} available coupons)
              </Text>
            </Space>
          }
          name="couponIds"
          rules={[
            { required: true, message: "Please select at least one coupon" },
          ]}
        >
          <Select
            mode="multiple"
        ...
        <Alert
          ...
          description={
            <div>
              <p>
                • Chỉ có thể gán các coupons đang hoạt động (is_active = true)
              </p>
              <p>• Tất cả coupons phải thuộc cùng một store</p>
              <p>
                • Coupons không thể được gán nếu đã liên kết với promotion khác
              </p>
              <p>
                • Thao tác này sẽ gán TẤT CẢ hoặc KHÔNG coupon nào (không cho
                phép gán một phần)
              </p>
            </div>
          }
